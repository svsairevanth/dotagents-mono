...
function normalizePromptForConfig(promptBody: string, defaultSystemPrompt: string): string {
  const bodyTrim = promptBody.trim()
  if (!bodyTrim) return ""
  if (bodyTrim === defaultSystemPrompt.trim()) return ""
  return promptBody
}
