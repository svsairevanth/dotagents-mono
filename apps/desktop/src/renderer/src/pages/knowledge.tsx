...
export function Component() {
  ...
  const consolidateMutation = useMutation({ mutationFn: async () => tipcClient.consolidateKnowledgeNotes(), onSuccess: (result) => { queryClient.invalidateQueries({ queryKey: ["knowledgeNotes"] }); if (result.reorganizedCount > 0) toast.success(`Reorganized ${result.reorganizedCount} notes into grouped collections`); else toast.success("No recurring notes needed reorganization"); if (result.errorCount > 0) toast.error(result.errors[0] || "Some notes could not be reorganized") }, onError: (error: Error) => toast.error(`Failed to reorganize notes: ${error.message}`) })
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            <Button variant="outline" size="sm" className="gap-2" onClick={() => consolidateMutation.mutate()} disabled={!knowledgeNotes.length || consolidateMutation.isPending}>{consolidateMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <FolderUp className="h-4 w-4" />}Organize Groups</Button>
        ...
        <details className="rounded-lg border bg-card"><summary className="cursor-pointer select-none px-4 py-3 text-sm font-medium">Knowledge note file template</summary><div className="px-4 pb-4 space-y-3"><p className="text-sm text-muted-foreground">DotAgents knowledge lives in plain files under <span className="font-mono">.agents/knowledge/</span>. Canonical note packages can stay flat at <span className="font-mono">.agents/knowledge/&lt;slug&gt;/&lt;slug&gt;.md</span> or use grouped folders like <span className="font-mono">.agents/knowledge/&lt;group&gt;/&lt;series&gt;/&lt;slug&gt;/&lt;slug&gt;.md</span>. Related assets like images or PDFs can live in the same note folder. Frontmatter uses simple <span className="font-mono">key: value</span> lines (not full YAML), and can optionally include <span className="font-mono">group</span>, <span className="font-mono">series</span>, and <span className="font-mono">entryType</span>. Most notes should use <span className="font-mono">context: search-only</span>; only a small curated subset should use <span className="font-mono">context: auto</span>.</p><div className="space-y-1"><div className="text-xs text-muted-foreground">Global layer: <span className="font-mono break-all">~/.agents/knowledge/</span></div><div className="text-xs text-muted-foreground">Workspace layer: <span className="font-mono break-all">./.agents/knowledge/</span></div><div className="text-xs text-muted-foreground">Workspace notes override global notes by <span className="font-mono">id</span> when both exist.</div></div><div className="rounded-md bg-muted p-3 font-mono text-xs whitespace-pre-wrap">{noteFileTemplate}</div></div></details>
        ...
        {knowledgeNotesQuery.isLoading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div> : filteredKnowledgeNotes.length === 0 ? <div className="rounded-lg border border-dashed bg-muted/20 px-5 py-6 text-center sm:px-6"><h3 className="text-base font-medium">No notes yet</h3><p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">{searchQuery ? "No notes match your search. Try a different query." : "Save notes from agent sessions to build your knowledge workspace."}</p></div> : <div className="space-y-6">{groupedKnowledgeNotes.map((group) => <div key={group.key} className="space-y-3"><div className="flex items-center gap-2 text-sm font-medium text-foreground"><FolderOpen className="h-4 w-4 text-muted-foreground" /><span>{group.label}</span><Badge variant="secondary" className="text-[10px]">{group.notes.length + group.seriesSections.reduce((sum, section) => sum + section.notes.length, 0)}</Badge></div>{group.notes.length ? <div className="space-y-3">{group.notes.map((note) => <KnowledgeNoteCard key={note.id} note={note} onDelete={(id) => setDeleteConfirmId(id)} onEdit={handleEdit} isSelected={selectedIds.has(note.id)} onToggleSelect={handleToggleSelect} />)}</div> : null}{group.seriesSections.map((section) => <div key={section.key} className="space-y-3 rounded-lg border border-dashed bg-muted/10 p-3"><div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-muted-foreground"><FolderUp className="h-3.5 w-3.5" /><span>{section.label}</span><Badge variant="secondary" className="text-[10px]">{section.notes.length}</Badge></div><div className="space-y-3">{section.notes.map((note) => <KnowledgeNoteCard key={note.id} note={note} onDelete={(id) => setDeleteConfirmId(id)} onEdit={handleEdit} isSelected={selectedIds.has(note.id)} onToggleSelect={handleToggleSelect} />)}</div></div>)}</div>)}</div>}
      </div>
